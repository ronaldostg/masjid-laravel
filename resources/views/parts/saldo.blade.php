...
<body class="bg-gray-50">
    ...
    <section class="w-full max-w-[22rem] lg:max-w-screen-xl mx-auto my-6">
        <div class="p-10">
            <h3 class="uppercase text-xl lg:text-3xl font-bold">Masjid Komunitas Bank Sumut</h3>
            ...
            <div class="w-full mt-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    {{-- {{ $ad->id }} --}}
                    <a href="/detail-saldo/{{ $ad->id }}"> 
                        <div id="container-masjid" class="h-40 shadow rounded-lg aktivitas-container">
                            <img class="w-full h-40 object-cover rounded-lg" src="{{ $ad->image_file ? 'https://mesjidadmin.unibroomindonesia.com/assets/files/upload/instansi/'.$ad->image_file : 'https://img.freepik.com/free-photo/muslim-landmark-red-travel-outdoor_1122-1627.jpg?t=st=1656491875~exp=1656492475~hmac=db26c0ca7a8646e6ada18d05d6337107818bc0623853db076c957cef53f718ad&w=740' }}" alt="Aktivitas 1" />

                            <div class="absolute bottom-0 ml-4 mb-4" style="zIndex: 2">
                                <h3 class="text-white font-semibold text-xl">{{ $ad->nama }}</h3>
                                <p class="text-xs text-white">{{ $ad->alamat }} | {{ $ad->no_hp }} | {{ $ad->email }}</p>
                            </div>
                    ...
                    {{-- {
                        ...
                        instances.data.datas_detail.map((item, index) => ( --}}


                            {{-- <div id="container-masjid" key={"index} onClick={() => detailMasjid(item.id, item.nama, item.alamat, item.no_hp, item.email, item.image_file ? `${process.env.urlfile}instansi/${item.image_file}` : 'https://img.freepik.com/free-photo/muslim-landmark-red-travel-outdoor_1122-1627.jpg?t=st=1656491875~exp=1656492475~hmac=db26c0ca7a8646e6ada18d05d6337107818bc0623853db076c957cef53f718ad&w=740')} className="h-40 shadow rounded-lg aktivitas-container">
                                <img className="w-full h-40 object-cover rounded-lg" src={item.image_file ? `${process.env.urlfile}instansi/${item.image_file}` : 'https://img.freepik.com/free-photo/muslim-landmark-red-travel-outdoor_1122-1627.jpg?t=st=1656491875~exp=1656492475~hmac=db26c0ca7a8646e6ada18d05d6337107818bc0623853db076c957cef53f718ad&w=740'} alt="Aktivitas 1" />

                                <div className="absolute bottom-0 ml-4 mb-4" style="zIndex: 2">
                                    <h3 className="text-white font-semibold text-xl">nama</h3>
                                    <p className="text-xs text-white">alamat | notelp | email</p>
                                </div>
                            </div> --}}
